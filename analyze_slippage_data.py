...
def plot_model_performance_evolution(df_perf):
    if df_perf.empty or 'num_training_samples' not in df_perf.columns:
        print("Performance log is empty or missing required columns for evolution plot.")
        return
    ...
    if not valid_mse.empty:
        ax1.plot(valid_mse['num_training_samples'], valid_mse['test_mse'], color=color, marker='.', linestyle='-', linewidth=1, markersize=4)
    ax1.tick_params(axis='y', labelcolor=color)
    ax1.set_xscale('log') # Log scale for training samples if it spans orders of magnitude
    ax1.set_yscale('log') 
    ...
    ax2.plot(df_perf['num_training_samples'], df_perf['test_r2_score'], color=color, marker='x', linestyle='--', linewidth=1, markersize=4)
    ax2.tick_params(axis='y', labelcolor=color)
    # R2 can be negative, so a fixed sensible range is good.
    min_r2 = min(-0.2, df_perf['test_r2_score'].min() - 0.05) if not df_perf.empty else -0.2
    max_r2 = max(1.0, df_perf['test_r2_score'].max() + 0.05) if not df_perf.empty else 1.0
...
def plot_feature_vs_slippage(df_probes):
    ...
    df_probes_filtered = df_probes[df_probes['true_slippage_pct_walk_the_book'].abs() < 1.0].copy()
    if len(df_probes_filtered) < 0.1 * len(df_probes): # If filtering removes too much, don't filter
        df_probes_filtered = df_probes.copy()
        print("Warning: Filtering for true_slippage_pct < 1% removed most data. Plotting all data.")
    
    print(f"Plotting feature vs slippage using {len(df_probes_filtered)} (potentially filtered) probe data points.")
    ...
    for feature in features_to_plot:
        ...
        sample_df = df_probes_filtered.sample(n=min(10000, len(df_probes_filtered)), random_state=1) if len(df_probes_filtered) > 10000 else df_probes_filtered
        ...
        if feature == 'probe_order_size_usd' or feature == 'market_depth_best_ask_usd':
            if sample_df[feature].max() / (sample_df[feature].min() + 1e-9) > 100: # Heuristic for log scale
                 use_log_x = True
        ...
        if std_slip > 0 : # Avoid issues if all slippage is identical
            ...
            upper_bound = median_slip + 5 * std_slip # Allow more space for positive slippage
            ...
            upper_bound = min(upper_bound, 0.1) # Cap maximum y at 0.1% if data is very concentrated
...
def plot_slippage_distribution(df_probes):
    ...
    if q_hi > q_low and (q_hi - q_low) < slippage_data_for_hist.std() * 10 : # Heuristic
        slippage_data_for_hist = slippage_data_for_hist[(slippage_data_for_hist >= q_low) & (slippage_data_for_hist <= q_hi)]

    if slippage_data_for_hist.empty:
        print("No slippage data left after filtering for histogram.")
        return

    sns.histplot(slippage_data_for_hist, kde=True, bins=100, stat="density") # Use density for KDE overlay
    ...
    plt.axvline(mean_slip, color='r', linestyle='--', linewidth=0.8, label=f'Mean: {mean_slip:.4f}%')
    plt.axvline(median_slip, color='g', linestyle=':', linewidth=0.8, label=f'Median: {median_slip:.4f}%')
    plt.legend()
...
def plot_predicted_vs_user_order_size(df_user_pred): # Renamed for clarity
    ...
    if 'user_order_size_usd' in df_user_pred.columns and 'predicted_slippage_pct_regression' in df_user_pred.columns:
        ...
        sample_df = df_user_pred.sample(n=min(10000, len(df_user_pred)), random_state=1) if len(df_user_pred) > 10000 else df_user_pred
        ...
        sns.scatterplot(data=sample_df, x='user_order_size_usd', y='predicted_slippage_pct_regression', alpha=0.3, s=15, edgecolor=None)
        
        # Check if x-axis (user_order_size_usd) needs log scale
        if sample_df['user_order_size_usd'].max() / (sample_df['user_order_size_usd'].min() + 1e-9) > 100:
            plt.xscale('log')
        ...
        print("User prediction data missing required columns for plotting (user_order_size_usd, predicted_slippage_pct_regression).")
...
def main():
    ...
    if not df_probes.empty:
        for col in ['probe_order_size_usd', 'market_spread_bps', 'market_depth_best_ask_usd', 'true_slippage_pct_walk_the_book']:
        ...
        if len(df_probes) < initial_probe_count:
            print(f"Filtered out {initial_probe_count - len(df_probes)} probe data points with negative spread_bps.")
    ...
    try:
        ...
        print(f"Warning: {PERFORMANCE_LOG_FILE} not found. Skipping performance evolution plot.")
    except Exception as e:
        print(f"Error loading or processing {PERFORMANCE_LOG_FILE}: {e}")

    # --- Generate Plots ---
    if not df_perf.empty: plot_model_performance_evolution(df_perf)
    if not df_probes.empty:
        plot_feature_vs_slippage(df_probes)
        plot_slippage_distribution(df_probes)
    if not df_user_pred.empty: plot_predicted_vs_user_order_size(df_user_pred)
